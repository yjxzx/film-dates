...
export default function UpdatesPage() {
  ...
  return (
    <div>
      ...
      <p className="text-[10px] text-ink-faded mb-8" style={{ fontFamily: "var(--font-newsprint-mono)" }}>
        数据来源：Git 提交历史 · 自动生成 ·{" "}
              ...
              <ul className="space-y-2">
                {grouped.get(date)!.map((entry, i) => (
                  <li key={i} className="flex gap-2 text-sm" style={{ fontFamily: "var(--font-newsprint-mono)" }}>
                    <span className={`shrink-0 text-[10px] px-1.5 py-0.5 border ${entry.type === "新增" ? "bg-ink text-newsprint border-ink" : "border-ink text-ink"}`}>
                      {entry.type}
                    </span>
